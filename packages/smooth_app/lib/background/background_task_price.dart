...
abstract class BackgroundTaskPrice extends BackgroundTask {
  ...
  BackgroundTaskPrice.fromJson(super.json)
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = _fromJsonListStringWithPadding(json[_jsonTagDiscountTypes], json.containsKey(_jsonTagBarcode) ? 1 : (_fromJsonListString(json[_jsonTagBarcodes])?.length ?? 0))!,
      super.fromJson();
  ...
  static List<String>? _fromJsonListStringWithPadding(final List<dynamic>? input, final int count) {
    if (input == null) {
  ...
  Future<void> addPrices({
    ...
    for (int i = 0; i < barcodes.length; i++) {
      ...
      final Price newPrice = Price()
        ...
        ..discountType = priceIsDiscounted ? DiscountType.fromOffTag(discountTypes[i]) : null
