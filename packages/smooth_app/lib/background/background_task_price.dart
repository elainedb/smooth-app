...
abstract class BackgroundTaskPrice extends BackgroundTask {
  ...
  BackgroundTaskPrice.fromJson(super.json)
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = _normalizeDiscountTypes(
        ...
        // Use barcodes length for normalization
        json.containsKey(_jsonTagBarcode)
            ? 1
            : (json[_jsonTagBarcodes] as List<dynamic>?)?.length ?? 0,
      ),
      super.fromJson();
  ...
  /// Normalizes the discount types list to match the expected [length].
  ///
  /// Defaults missing or null entries to empty strings for backward
  /// compatibility with tasks serialized before this field existed.
  static List<String> _normalizeDiscountTypes(
    ...
    return <String>[
      ...input,
      ...List<String>.filled(length - input.length, ''),
    ];
  }
  ...
  Future<void> addPrices({
    ...
    for (int i = 0; i < barcodes.length; i++) {
      ...
      final Price newPrice = Price()
        ...
        ..discountType = priceIsDiscounted && discountTypes[i].isNotEmpty
