...
abstract class BackgroundTaskPrice extends BackgroundTask {
    ...
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = (() {
        final List<String> list =
            _fromJsonListString(json[_jsonTagDiscountTypes]) ?? <String>[];
        final int targetLength = json.containsKey(_jsonTagBarcode)
            ? 1
            : (_fromJsonListString(json[_jsonTagBarcodes])?.length ?? 0);
  ...
  Future<void> addPrices({
    ...
    for (int i = 0; i < barcodes.length; i++) {
      ...
      final String currentDiscountType = discountTypes[i];
      if (priceIsDiscounted && currentDiscountType.isNotEmpty) {
        newPrice.discountType = DiscountType.fromOffTag(currentDiscountType);
      }
