...
abstract class BackgroundTaskPrice extends BackgroundTask {
  ...
  BackgroundTaskPrice.fromJson(super.json)
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = _fromJsonListStringPad(
        json[_jsonTagDiscountTypes],
        (json.containsKey(_jsonTagBarcode) ? 1 : (_fromJsonListString(json[_jsonTagBarcodes])?.length ?? 0)),
      ),
      super.fromJson();
  ...
  static List<String> _fromJsonListStringPad(final List<dynamic>? input, int expectedLength) {
    final List<String> result = _fromJsonListString(input) ?? <String>[];
  ...
  Future<void> addPrices({
    ...
    for (int i = 0; i < barcodes.length; i++) {
      ...
      if (priceIsDiscounted && discountTypes.length > i && discountTypes[i].isNotEmpty) {
        newPrice.discountType = DiscountType.fromOffTag(discountTypes[i]);
      }
