...
abstract class BackgroundTaskPrice extends BackgroundTask {
  ...
  BackgroundTaskPrice.fromJson(super.json)
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = _fromJsonListString(json[_jsonTagDiscountTypes]) ??
          <String>[],
      super.fromJson();
  ...
  /// Returns the discount type for item at [index], normalizing for backward
  /// compatibility (missing/short lists default to empty string).
  String _discountTypeAt(int index) =>
      index < discountTypes.length ? discountTypes[index] : '';
  ...
  Future<void> addPrices({
    ...
    if (split) {
      for (int i = 0; i < barcodes.length; i++) {
        ...
        await BackgroundTaskAddOtherPrice.addTask(
          ...
          discountTypes: <String>[_discountTypeAt(i)],
        );
      }
      return;
    }
    ...
    for (int i = 0; i < barcodes.length; i++) {
      ...
      final String discountTypeTag = _discountTypeAt(i);
      final Price newPrice = Price()
        ...
        ..discountType = priceIsDiscounted && discountTypeTag.isNotEmpty
            ? DiscountType.fromOffTag(discountTypeTag)
            : null;
