...
abstract class BackgroundTaskPrice extends BackgroundTask {
  ...
  BackgroundTaskPrice.fromJson(super.json)
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = _fromJsonListString(json[_jsonTagDiscountTypes]) ?? List<String>.filled((json[_jsonTagBarcodes] as List<dynamic>?)?.length ?? (json.containsKey(_jsonTagBarcode) ? 1 : 0), ''),
      super.fromJson();
  ...
  Future<void> addPrices({
    ...
    for (int i = 0; i < barcodes.length; i++) {
      ...
      final Price newPrice = Price()
        ...
        ..discountType = priceIsDiscounted && discountTypes[i].isNotEmpty ? DiscountType.fromOffTag(discountTypes[i]) : null
