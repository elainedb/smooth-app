...
abstract class BackgroundTaskPrice extends BackgroundTask {
    ...
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = _normalizeDiscountTypes(
        ...
        json.containsKey(_jsonTagIsDiscounted)
            ? 1
            : (json[_jsonTagAreDiscounted] as List<dynamic>?)?.length ?? 0,
      ),
  ...
  /// Normalizes [discountTypes] to match [barcodesCount], defaulting to `''`.
  static List<String> _normalizeDiscountTypes(
    final List<String>? input,
    final int barcodesCount,
  ) {
    if (input == null) {
      return List<String>.filled(barcodesCount, '');
    }
    if (input.length >= barcodesCount) {
      return input.sublist(0, barcodesCount);
    }
    return <String>[
      ...input,
      ...List<String>.filled(barcodesCount - input.length, ''),
    ];
  }
