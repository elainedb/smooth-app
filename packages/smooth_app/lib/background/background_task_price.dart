...
abstract class BackgroundTaskPrice extends BackgroundTask {
  ...
  BackgroundTaskPrice.fromJson(super.json)
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = _normalizeDiscountTypes(
        ...
        json.containsKey(_jsonTagIsDiscounted)
            ? 1
            : (_fromJsonListBool(json[_jsonTagAreDiscounted])?.length ?? 0),
      ),
  ...
  ///
  /// Backward compatibility: if the list is null or shorter than expected,
  /// missing entries default to empty string (no discount type selected).
