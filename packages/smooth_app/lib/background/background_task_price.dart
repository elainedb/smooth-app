...
abstract class BackgroundTaskPrice extends BackgroundTask {
  ...
  BackgroundTaskPrice.fromJson(super.json)
    : date = JsonHelper.stringTimestampToDate(json[_jsonTagDate] as String),
      ...
      discountTypes = _getDiscountTypesFrom(json),
      super.fromJson();

  static List<String> _getDiscountTypesFrom(Map<String, dynamic> json) {
    final List<String>? parsed = _fromJsonListString(json[_jsonTagDiscountTypes]);
    if (parsed != null) {
      return parsed;
    }
    final int count;
    if (json.containsKey(_jsonTagBarcode)) {
      count = 1;
    } else {
      count = _fromJsonListString(json[_jsonTagBarcodes])?.length ?? 0;
    }
  ...
  Future<void> addPrices({
    ...
    for (int i = 0; i < barcodes.length; i++) {
      ...
      if (priceIsDiscounted && discountTypes[i].isNotEmpty) {
